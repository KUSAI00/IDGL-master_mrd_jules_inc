...
def compute_anchor_adj(node_anchor_adj, anchor_mask=None):
    ...
    if anchor_mask is not None:
        anchor_adj = anchor_adj.masked_fill((anchor_mask == 0).unsqueeze(-1).bool(), markoff_value)
        anchor_adj = anchor_adj.masked_fill((anchor_mask == 0).unsqueeze(-2).bool(), markoff_value)
