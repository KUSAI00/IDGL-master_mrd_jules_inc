    ...
    def __init__(self, instances, config, word_vocab):
        ...
        if config.get('use_distilbert', False):
            ...
            if _bert_tokenizer is None:
                from .bert_utils import DistilBertTokenizerWrapper
                _bert_tokenizer = DistilBertTokenizerWrapper()
            tokenizer = _bert_tokenizer

            for instance in self.instances:
                # DistilBert tokenization
                sent1_cut = instance[0]  # List of words
                ids = tokenizer.encode(sent1_cut, max_length=config.get('max_seq_len', 512))
                self.sent1_word.append(ids)
                self.sent1_length.append(len(ids))
                if self.has_sent2:
                    sent2_cut = instance[1]
                    ids2 = tokenizer.encode(sent2_cut, max_length=config.get('max_seq_len', 512))
                    self.sent2_word.append(ids2)
                    self.sent2_length.append(len(ids2))
